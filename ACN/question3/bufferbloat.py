...
def start_iperf(net):
    ...
    #h1.popen("iperf -c %s -t %s -P 10 > %s/iperf.out" % (h2.IP(), args.time, args.dir), shell=True)

    #starting udp server.
    h2.popen("iperf -s -u -p 5566 -i 1 > %s/server.out"%(args.dir),shell =True)
    h1.popen("iperf -c %s -u -b 10M -t 25 -p 5566  > %s/iperf1.out" % (h2.IP(), args.dir), shell=True)
...
def start_ping(net):
    ...
    #h3 = net.get('h3')
    #popen = h1.popen("ping -i 0.1 %s > %s/ping1.txt"%(h2.IP(), args.dir), shell=True)
